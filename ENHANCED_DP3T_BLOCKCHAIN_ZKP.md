...
). We believe that all these issues, or at least most of them, no longer appear in a totally decentralized solution that do not requires the central back-end/server that focalizes most of the issues as is a single point of failure and eavesdropping.
...
There are different ways to achieve peer-to-peer between individuals. A blockchain-based one (using Ethereum technology) is the [whisper protocol]( https://github.com/ethereum/wiki/wiki/Whisper). This allows peer-to-peer communication between Dapps.
