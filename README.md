...
* Generate a map where pseudonymous information about citizens is displayed with different colors depending on if they are not infected (white), with confirmed COVID-19 (yellow), or recovered (green). Additionally, dahsboards will show total numbers of engagement and other useful information. The only information that is collected is age, sex, location, and information to notarize.
...
And the API documentation to consume information collected from the public smart contract is [HERE](https://github.com/lacchain/DAVID19-taskforce/blob/master/backend-api.md).
...
*	White for healthy
* Yellow for infection confirmed
* Green for recovered
...
* Sex
* Age
* Location of the person with a precision of 1 km square
